...
namespace NonConTroll.Generators
{
    [Generator]
    public class SyntaxNodeGetChildrenGenerator : ISourceGenerator
    {
        ...
        public void Execute( SourceGeneratorContext context )
        {
            ...
            indentedTextWriter.WriteLine( "namespace NonConTroll.CodeAnalysis.Syntax" );
            indentedTextWriter.WriteLine( "{" );
            indentedTextWriter.Indent++;

            foreach( var type in syntaxNodeTypes )
            {
                indentedTextWriter.WriteLine( $"public partial class {type.Name}" );
                indentedTextWriter.WriteLine( "{" );
                indentedTextWriter.Indent++;
                ...
                foreach( var property in properties )
                {
                    if( property.Type is INamedTypeSymbol propertyType )
                    {
                        ...
                        {
                            indentedTextWriter.WriteLine( $"foreach( var child in this.{property.Name} )" );
                            indentedTextWriter.Indent++;
                            ...
                            isEmpty = false;
                                 ...
                                 this.IsDerivedFrom( propertyType.TypeArguments[ 0 ] , syntaxNodeType ) )
                        {
                            indentedTextWriter.WriteLine( $"foreach( var child in this.{property.Name}.GetNodesWithSeparators() )" );
                            indentedTextWriter.Indent++;
                            indentedTextWriter.WriteLine( "yield return child;" );
                            indentedTextWriter.Indent--;
        ...
        private IReadOnlyList<INamedTypeSymbol> GetAllTypes( IAssemblySymbol symbol )
        {
            ...
            this.GetAllTypes( result , symbol.GlobalNamespace );
        ...
        private void GetAllTypes( List<INamedTypeSymbol> result , INamespaceOrTypeSymbol symbol )
        {
            ...
            foreach( var child in symbol.GetMembers() )
            {
                ...
                {
                    this.GetAllTypes( result , nsChild );
                }
            }
        }
